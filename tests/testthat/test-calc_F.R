...
nfleets <- 2
test_timeseries <- data.frame(Area = 1,
                              Yr = rep(-1:4, 2),
...
test_that("get_F works with multi yrs, fleets, and seasons", {
  ...
  expect_equal(F_list$F_rate$name,
   c("F_fleet_1_YR_1_s_1", "F_fleet_1_YR_1_s_2", "F_fleet_1_YR_2_s_1",
  ...
  expect_equal(names(F_list$init_F),
               c("InitF_seas_1_flt_1Fishery", "InitF_seas_2_flt_1Fishery",
                 "InitF_seas_2_flt_2Survey"))
  expect_equal(F_list$F_rate_fcast$name,
...
test_that("get_F can catch bad input", {
  ...
  expect_error(get_F(test_timeseries, fleetnames_wrong_len),
               "is_of_length : fleetnames has length 3, not 2.", fixed = TRUE)
...
test_that("get_retained_catch works", {
  ...
  expect_equal(colnames(ret_catch),
