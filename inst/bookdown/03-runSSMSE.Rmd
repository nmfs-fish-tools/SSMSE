...
There are `r length(built_in_mods)` built-in models that comes with SSMSE: `r built_in_mods_final`. To use a built in model, specify the name of the model as part of the `OM_name_vec`. For example, to use the cod model in run_SSMSE for 1 scenario0, set `OM_name_vec = "cod"`. Otherwise, the path to the OM model should be specified in `OM_in_dir_vec`.
...
### Example of future_om_list with a random deviations
