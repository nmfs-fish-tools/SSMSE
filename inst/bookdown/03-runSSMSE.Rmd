...
By default, SSMSE will simply extend the structure of the OM into the future using the same parameter values as in the last year of the model. However, it is likely that the user will want to allow changes in parameter values to occur as the OM is extended forward in time. User can input values into the `future_om_list` to accomplish this. This input is a list of lists. The first level of lists specify 1 type of change. Within the first level, there are 4 list components. There are 2 main choices: 1) to specify model changes by telling SSMSE how it should sample values or 2) to input your own custom values for a given parameter. 
...
* ts_param: This is the parameter to  modify. Options are "mean", "ar_1_phi" (if using autocorrelation), "sd", and "cv". *(note: make sure cv is implemented)* If not included in the dataframe, "mean" is assumed to be the same as in the previous model year, sd is assumed to be 0, and we assume no autocorrelation (as specified through an autoregressive AR1 process). Phi should be between -1 and 1 (this creates a stationary process) and if phi = 1, this creates a random walk (as random walk is a special case of an AR1 model).
...
This example applies a random jitter to all parametero for scenario scen2 and 
scen3.
...
* iter: which number iteration the value should apply to. These are relative iteration numbers rather than absolute (e.g., if the first iteration being run through the run_SSMSE call is 30, then iteration = 1 would specify the value for iteration 30). 
