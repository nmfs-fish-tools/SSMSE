...
#                                option="default", ...) # or option = "damping", related to how c reduces TAC change. 
#   ## NOTE: only configured for depletion basis = 1 or 2. 
# {
#   
#   # B_ratio_denominator
...
#   if(SSout$depletion_basis == 3 | SSout$depletion_basis==4 | SSout$depletion_basis==2){simpleError("ERROR -- depletion basis 1 or 2")}
...
#   # get the fleets and the units on catch
#   units <- dat[["fleetinfo"]]
#   units[["survey_number"]] <- seq_len(nrow(units)) # append survey number column onto units/fleetinfo
#   flt_units <- units[units[["type"]] %in% c(1, 2), c("survey_number", "units")] # get fleet number and catch units
#   # for multi-area models, need to summarize across areas (note a fleet
#   # only operates in 1 area)
#   # may also need to consider if the catch multiplier is used.
#   # match catch with the units
#   unit_key <- data.frame(unit_name = c("B", "N"), units = 1:2)
#   flt_units <- merge(flt_units, unit_key, all.x = TRUE, all.y = FALSE) # append unit key / name to fleet units
#   # get the se
...
#     New_catch<-LYr_catch * sratio
#     
#     if(option=="default"){TAC<- ( (New_catch - LYr_catch) * c1 ) + LYr_catch}
#     if(option=="damping"){TAC <- LYr_catch + exp(log(New_catch)*c1)}
#     
#     df_list[[fl]] <- data.frame(
#       area = fcast_catch_df[["Area"]],
#       year = fcast_catch_df[["Yr"]],
...
#     New_catch_bio<-LYr_catch_bio * sratio
#     if(option=="default"){TAC_bio <- ( (New_catch_bio - LYr_catch_bio) * c1 ) + LYr_catch_bio}
#     if(option=="damping"){TAC_bio <- LYr_catch_bio + exp(log(New_catch_bio)*c1)}
#     bio_df_list[[fl]] <- data.frame(
#       area = fcast_catch_df[["Area"]],
#       year = fcast_catch_df[["Yr"]],
#       seas = fcast_catch_df[["Seas"]],
...
#   catch_df <- as.data.frame(catch_df)
#   catch_bio_df <- as.data.frame(catch_bio_df)
#   catch_F_df <- NULL #as.data.frame(catch_F_df)
#   
#   # get discard, if necessary
#   if (dat[["N_discard_fleets"]] > 0) {
#     # discard units: 1, biomass/number according to set in catch
#     # 2, value are fraction (biomass/numbers ) of total catch discarded
#     # 3, values are in numbers(thousands)
#     se_dis <- SSMSE:::get_input_value(dat[["discard_data"]],
#                                       method = "most_common_value",
#                                       colname = "Std_in", group = "Flt"
#     )
#     dis_df_list <- vector(
...
#           )
#         }
#         dis_df_list[[i]] <- data.frame( #### specifying discards isn't doing anything; don't read in discards to SS, uses retention function. 
#           Yr = fcast_catch_df[["Yr"]],
#           Seas = fcast_catch_df[["Seas"]],
#           Flt = tmp_flt,
#           Discard = tmp_discard_amount,
...
#     catch_F = catch_F_df
#   )
#   # return(list(new_dat_list=new_dat_list, sratio=sratio))
# }
# 
# # new_catch_list <- get_RatioEM_catch_df(EM_dir = EM_out_dir, dat = new_EM_dat, 
# #                                        changeup=0.2, changedown=0.2,# add inputs, min & max allowable annual percent change in catch
# #                                        c=0.75,# add inputs, multiplicative constant 
# #                                        option="default")
# # new_catch_list
# 
# 
# RatioEM <- function(EM_out_dir = NULL, init_loop = TRUE, OM_dat, verbose = FALSE,
#                     nyrs_assess, dat_yrs, sample_struct = NULL, seed = NULL, OM_out_dir, ...) {
...
#   run_EM(EM_dir = EM_out_dir, verbose = verbose, check_converged = TRUE)
#   # get the forecasted catch.
#   new_catch_list <- get_RatioEM_catch_df(EM_dir = EM_out_dir, dat = new_EM_dat, dat_yrs=dat_yrs)
#   # new_catch_list <- get_EM_catch_df(EM_dir = EM_out_dir, dat = new_EM_dat)
#   new_catch_list
# }
# 
