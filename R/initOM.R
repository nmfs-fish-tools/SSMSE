...
create_OM <- function(OM_out_dir,
  ...
  if (add_dummy_dat) {
    ...
    if (verbose) {
      message(
        "Input uncertainty for CPUE OM currently can only have a single ",
    ...
    if (dat[["use_lencomp"]] == 1) {
      ...
      if (verbose) {
        message(
          "Input uncertainty for lencomp OM currently can only have a ",
      ...
      for (i in seq_len(nrow(lcomp_combo))) {
        ...
        # safely ignored
        suppressWarnings(tmp_df_lencomp <- data.frame(
          Yr = tmp_miss_yrs_lencomp,
    ...
    meta_cols_agecomp <- c(
      "Seas", "FltSvy", "Gender", "Part", "Ageerr",
      "Lbin_lo", "Lbin_hi"
    )
    agecomp_combo <- unique(dat[["agecomp"]][, meta_cols_agecomp])
    # get the Nsamp
    # TODO: add more options for getting sample size.
    age_Nsamp_val <- get_input_value(
      data = old_agecomp,
      method = "most_common_value",
      colname = "Nsamp",
      group = "FltSvy"
    ...
    if (verbose) {
      message(
        "Input uncertainty for agecomp OM currently can only have a ",
    ...
    for (i in 1:nrow(agecomp_combo)) {
      ...
      # used suppressWarning because creates an unwanted warning that can be
      # safely ignored
      suppressWarnings(tmp_df_agecomp <- data.frame(
        Yr = tmp_miss_yrs_agecomp,
        Seas = tmp_metacols["Seas"],
        FltSvy = -tmp_metacols["FltSvy"],
        Gender = tmp_metacols["Gender"],
        Part = tmp_metacols["Part"],
