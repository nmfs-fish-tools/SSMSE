...
#'   if using a custom model with path specified in \code{OM_dir}.
...
run_SSMSE_iter <- function(OM_name     = "cod", 
  ...
  if(!is.null(OM_name) && !OM_name %in% pkg_mods) {
  ...
  OM_in_dir <- pkg_dirs[grep(OM_name, pkg_dirs, fixed = TRUE)]
  }
  OM_dir <- file.path(out_dir, paste0(basename(OM_in_dir), "_OM"))
  dir.create(OM_dir)
  # MSE first iteration ----
  # turn the stock assessment model into an OM
  create_OM(OM_dir = OM_dir, SA_dir = OM_in_dir, overwrite = TRUE, add_dummy_dat = FALSE,
